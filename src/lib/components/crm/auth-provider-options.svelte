<script lang="ts">
	...
	const providers = (env.PUBLIC_AUTH_PROVIDERS ?? '').split(',').map((value) => value.trim());
...
{#if providers.some((value) => ['google', 'azure', 'sso'].includes(value))}
	<div class="space-y-4">
		{#if errorMessage}
			<p class="text-sm text-destructive" role="alert">{errorMessage}</p>
		{/if}

		{#if providers.includes('sso')}
		...
		<div class="grid gap-2 sm:grid-cols-2">
			...
			{#if providers.includes('azure')}
				...
				>
					...
					{pending === 'azure' ? 'Connecting…' : 'Microsoft'}
				</Button>
			{/if}
		</div>
	</div>
