<script lang="ts">
	...
	import EmailTemplatesListPage from './email-templates-list-page.svelte';
	...
	let {
		...
		onOpenNew,
		class: className
	}: EmailTemplatesPageProps = $props();

	let viewState = $state<ResourceViewState>({ kind: 'loading' });
	let rows = $state<EmailTemplateRow[]>([]);
	...
	async function loadAll() {
		...
		try {
			...
			const listed = await api.emailTemplates.list({ limit: 100 });
			if (isStale(epoch)) return;

			rows = listed.data.map(toEmailTemplateRow);
			viewState =
				rows.length === 0
					? { kind: 'empty', message: 'No templates yet — create your first reusable email.' }
					: { kind: 'ready' };
		} catch (error) {
			if (isStale(epoch)) return;
			...
			viewState = {
				kind: 'validation',
				message: userMessage(error, 'Could not load email templates.')
			};
		}
	}
	...
	async function onCreateTemplate(): Promise<boolean> {
		...
		try {
			...
			rows = [toEmailTemplateRow(created), ...rows];
			...
			viewState = {
				kind: 'validation',
				message: userMessage(error, 'Could not create template — try again.'),
...
{#if currentOrgId}
	<div class={className} data-testid="email-templates-page">
		<AppShell
			{currentOrgId}
		...
		>
			<div class="flex min-h-0 flex-1 flex-col">
				...
				<EmailTemplatesListPage
					...
					{viewState}
					onReload={loadAll}
					onValidSubmit={onCreateTemplate}
					onNewTemplate={onOpenNew}
					showNav={false}
...
{:else}
	<div class="p-6" data-testid="email-templates-page">
