...
# Invitation mail goes to local Mailpit (config.toml [local_smtp] smtp_port = 54325).
SYSTEM_SMTP_HOST="${SYSTEM_SMTP_HOST:-host.docker.internal}"
SYSTEM_SMTP_PORT="${SYSTEM_SMTP_PORT:-54325}"
SYSTEM_SMTP_SECURITY="${SYSTEM_SMTP_SECURITY:-none}"
SYSTEM_SMTP_FROM="${SYSTEM_SMTP_FROM:-headquarters@localhost}"
...
SYSTEM_SMTP_SECURITY=${SYSTEM_SMTP_SECURITY}
SYSTEM_SMTP_FROM=${SYSTEM_SMTP_FROM}
EOF
...
SYSTEM_SMTP_FROM=${SYSTEM_SMTP_FROM}
EOF
log "wrote PUBLIC_SUPABASE_URL + system SMTP (Mailpit) + APP_BASE_URL into supabase/functions/.env"
