-- Allow task-proposal accept after a meeting is completed (post-meeting wrap-up:
-- transcript → summary → accept). Keep cancelled meetings blocked.
...
begin
  ...
  if v_meeting.status = 'cancelled' then
    raise exception 'This meeting was cancelled, so follow-up tasks cannot be accepted.'
      using errcode = 'P0001';
  end if;
