-- Wave B.1: draft synthesis must not read private mailbox bodies for
-- non-owners. Require mailbox owner OR an existing timeline_share link
-- (same spirit as email_messages_select_owner_or_share). Fail with 404
-- (P0002) when not visible so private mailboxes do not leak existence.
