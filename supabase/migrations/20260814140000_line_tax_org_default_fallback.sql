-- Apply org default tax when line tax_rate_percent is omitted.
-- Resolution: explicit tax_rate_percent (incl. 0) > product active tax rate > org default > 0.
-- Aligns API/MCP/RPC with CRM UI blank-line seeding of tax_rates.is_default.

create or replace function private.org_default_tax_rate_percent(p_org_id uuid)
returns numeric
language sql
stable
security definer
...
as $$
  select coalesce(
    (
      ...
      where tax_rates.org_id = p_org_id
...
revoke all on function private.org_default_tax_rate_percent(uuid)
...
revoke all on function private.resolve_document_line_tax_rate(uuid, jsonb, uuid)
  from public, anon, authenticated;
