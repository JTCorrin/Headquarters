  ...
  if ("provider_payment_id" in body) {
    if (body.provider_payment_id === null) providerPaymentId = null;
      ...
      fields.provider_payment_id =
        "Must be a string up to 200 characters or null";
    } else {
...
function asPaymentDocument(data: Json): PaymentDocument {
  ...
  return {
    ...
    ...(payload.reversing_payment
      ? { reversing_payment: payload.reversing_payment }
      : {}),
  };
}
...
async function listPayments(
  ...
  const { data, error } = await db.rpc("list_payments", {
    ...
    p_invoice_id: invoiceIdParam
      ? parseUuid(invoiceIdParam, "invoice_id")
      : null,
