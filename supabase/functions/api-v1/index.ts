...
async function routeOrgScoped(
  ...
  if (path === '/api/v1/api-keys' || path.startsWith('/api/v1/api-keys/')) {
    ...
    return await handleApiKeys(req, db, path, orgId, membership.role, requestId)
  }
  ...
  if (path === '/api/v1/products' || path.startsWith('/api/v1/products/')) {
    assertCanAccessCatalog(membership.role, req.method)
    return await handleProducts(req, db, path, orgId, requestId, requireUserId(userId))
  }
...
async function handleApiKeyRequest(req: Request, requestId: string): Promise<Response> {
  const path = apiPath(new URL(req.url).pathname)
  if (path === '/api/v1/health' && req.method === 'GET') {
    return jsonResponse({ data: { status: 'ok', api_version: 'v1' } }, 200, requestId)
  }
