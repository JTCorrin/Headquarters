...
function cursorFromClaim(claimed: Record<string, unknown>): MailboxSyncCursor {
  const uidvalidity = claimed.imap_uidvalidity == null
    ? null
    : Number(claimed.imap_uidvalidity);
  return {
    uidvalidity: Number.isFinite(uidvalidity) && uidvalidity! > 0
      ? uidvalidity
      : null,
    highUid: claimed.sync_high_uid == null
      ? null
      : Number(claimed.sync_high_uid),
...
export async function runMailboxSyncCycle(
  ...
  let highUid = claimed.sync_high_uid == null
    ? null
    : Number(claimed.sync_high_uid);
  let lowUid = claimed.sync_low_uid == null
    ? null
    : Number(claimed.sync_low_uid);
    ...
    if (!claimed.credentials_configured) {
      ...
      if (!resolved) {
        ...
        const security: ImapSecurity =
          securityRaw === "starttls" || securityRaw === "none"
            ? securityRaw
            : "tls";
