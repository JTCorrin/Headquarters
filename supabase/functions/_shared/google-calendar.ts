...
export function isCalendarSyncStubMode(
  ...
  const flag = (getEnv('CALENDAR_SYNC_STAGING_STUB') ?? '').trim().toLowerCase()
...
function eventBody(event: GoogleEventInput): Record<string, unknown> {
  ...
  if (event.meeting_url) descriptionParts.push(`Meeting URL: ${event.meeting_url}`)
...
export function createStubGoogleCalendarClient(meetingIdForStub?: string): GoogleCalendarClient {
  return {
    async insertEvent(_calendarId, _event) {
      const suffix = meetingIdForStub ?? crypto.randomUUID()
      return { id: `stub-${suffix}` }
    },
    async patchEvent(_calendarId, eventId, _event) {
      return { id: eventId }
    },
...
async function ensureAccessToken(
  ...
  if (blob.access_token && Number.isFinite(expiryMs) && expiryMs > now + 60_000) {
