...
export type ImapFetchOptions = {
  ...
  onProgress?: (event: {
    ...
    cursor: MailboxSyncCursor;
    uidvalidity: number;
    mode: "first" | "newer" | "older";
  }) => Promise<void>;
} & ImapTimeoutOptions;

export type ImapInboxSyncResult = {
 ...
 * Capped at 5 so Outlook/Mailcow FETCH literals stay framed while cutting round-trips.
 */
...
/** Decode base64 body (whitespace ignored). Truncated IMAP TEXT must not throw. */
...
function decodeTransferEncoding(
  ...
  return bytesToText(decodeTransferEncodingBytes(raw, cte), charset);
}
...
const INLINE_IMAGE_MAX_BYTES = 100_000;
...
function isMultipartType(contentType: string): boolean {
...
function filenameFromPartHeaders(
  headers: Record<string, string>,
  contentType: string,
): string {
  ...
  const name = contentType.match(/name\s*=\s*"?([^";]+)"?/i);
  if (name?.[1]) return name[1].trim();
  return "attachment";
}

function splitMultipart(body: string, boundary: string): MimeNode[] {
  ...
  for (const segment of segments) {
    ...
    const rawBody = segment.slice(headerEnd).replace(/^\r?\n\r?\n/, "").replace(
      /^\r?\n/,
      "",
    );
    nodes.push({ headers, rawBody });
  }
  return nodes;
}
...
function collectMime(
  ...
  const dispo = (node.headers["content-disposition"] ?? "").toLowerCase();

  if (isMultipartType(contentType)) {
    const boundary = boundaryFromContentType(contentType);
    if (!boundary) return;
    for (const child of splitMultipart(node.rawBody, boundary)) {
      collectMime(child, acc);
    }
    return;
  }

  const decodedBytes = decodeTransferEncodingBytes(
    node.rawBody,
    node.headers["content-transfer-encoding"],
  );
  const charset = charsetFromContentType(contentType);
  const isAttachment = dispo.includes("attachment");

  if (ctLower.includes("text/plain") && !isAttachment) {
    const text = bytesToText(decodedBytes, charset).replace(/\s+$/u, "");
    if (acc.plain === null && text) acc.plain = text;
    return;
  }
  if (ctLower.includes("text/html") && !isAttachment) {
    const html = bytesToText(decodedBytes, charset);
    if (acc.html === null && html.trim()) acc.html = html;
    return;
  }

  const filename = filenameFromPartHeaders(node.headers, contentType);
  const cidRaw = (node.headers["content-id"] ?? "").replace(/^<|>$/g, "")
    .trim();
  ...
  acc.attachments.push({
    ...
    content_id: cidRaw || null,
    inline: Boolean(cidRaw) || (dispo.includes("inline") && !isAttachment),
  });
  ...
  if (
    ...
    decodedBytes.length <= INLINE_IMAGE_MAX_BYTES
  ) {
    acc.cid.set(
      cidRaw.toLowerCase(),
      `data:${mediaType};base64,${bytesToBase64(decodedBytes)}`,
    );
  }
}

/** Decode IMAP BODY[TEXT] plus optional envelope Content-Type / CTE. */
export function decodeMimeMessage(
  rawBody: string,
  headerContentType?: string,
  headerCte?: string,
): DecodedMimeBody {
  const acc = {
    plain: null as string | null,
    html: null as string | null,
    attachments: [] as MailboxAttachmentMeta[],
    cid: new Map<string, string>(),
  };
  const trimmed = rawBody.replace(/^\uFEFF/, "");
  const ct = headerContentType ?? "";
  if (isMultipartType(ct) || looksLikeMultipartBody(trimmed)) {
    const firstLine = trimmed.replace(/^\r?\n+/, "").match(/^--([^\r\n]+)/);
    let boundary = boundaryFromContentType(ct);
    if (!boundary && firstLine?.[1]) {
      boundary = firstLine[1].endsWith("--")
        ? firstLine[1].slice(0, -2)
        : firstLine[1];
    }
    if (boundary) {
      collectMime(
        {
          headers: {
            "content-type": ct || `multipart/mixed; boundary="${boundary}"`,
          },
          rawBody: trimmed,
        },
        acc,
    ...
    const decoded = decodeTransferEncoding(
      trimmed,
      headerCte,
      charsetFromContentType(ct),
    );
    if (ct.toLowerCase().includes("text/html")) acc.html = decoded;
    else acc.plain = decoded;
  }

  let html = acc.html;
  if (html && acc.cid.size > 0) {
    html = html.replace(/cid:([^"'\s>]+)/gi, (all, id: string) => {
      const key = String(id).replace(/^<|>$/g, "").trim().toLowerCase();
      return acc.cid.get(key) ?? all;
    });
  }

  const text = acc.plain && acc.plain.length > 0
    ? acc.plain
    : html
    ? stripHtmlToText(html)
    : trimmed && !looksLikeMultipartBody(trimmed)
    ? trimmed
    : acc.plain ?? "";

  return { text, html, attachments: acc.attachments };
}

/**
 * Turn IMAP BODY[TEXT] into displayable plain text.
 * Prefers nested multipart text/plain; decodes quoted-printable / base64; HTML→text fallback.
 */
export function decodeMimeBodyText(
  raw: string,
  headerContentType?: string,
  headerCte?: string,
): string {
  if (!raw) return "";
  return decodeMimeMessage(raw, headerContentType, headerCte).text;
}

export function emptyMailboxSyncCursor(
...
export function parseUidValidity(
  untagged: string[],
  statusText = "",
): number | null {
  const blob = [...untagged, statusText].join("\n");
  const m = blob.match(/UIDVALIDITY\s+(\d+)/i);
  if (!m) return null;
  const n = Number(m[1]);
  return Number.isFinite(n) && n > 0 ? n : null;
}

export function selectUidsForCycle(
  allUids: number[],
  cursor: MailboxSyncCursor,
  budget: number,
): { uids: number[]; mode: "first" | "newer" | "older" | "idle" } {
  const sorted = [
    ...new Set(allUids.filter((u) => Number.isFinite(u) && u > 0)),
  ]
    .sort((a, b) => a - b);
  const n = Math.max(1, Math.floor(budget));
  if (sorted.length === 0) return { uids: [], mode: "idle" };

  if (cursor.catchupComplete) {
    if (cursor.highUid == null) {
      return { uids: sorted.slice(-n), mode: "first" };
    }
    const highUid = cursor.highUid;
    const newer = sorted.filter((u) => u > highUid);
    return { uids: newer.slice(-n), mode: newer.length > 0 ? "newer" : "idle" };
  }

  if (cursor.highUid == null || cursor.lowUid == null) {
    return { uids: sorted.slice(-n), mode: "first" };
  }

  const highUid = cursor.highUid;
  const lowUid = cursor.lowUid;
  const newer = sorted.filter((u) => u > highUid);
  if (newer.length > 0) {
    return { uids: newer.slice(-n), mode: "newer" };
  }
  const older = sorted.filter((u) => u < lowUid);
  if (older.length === 0) return { uids: [], mode: "idle" };
  return { uids: older.slice(-n), mode: "older" };
}

export function applyFetchedUid(
  cursor: MailboxSyncCursor,
  uid: number,
  mode: "first" | "newer" | "older",
): MailboxSyncCursor {
  if (mode === "newer") {
    return { ...cursor, highUid: Math.max(cursor.highUid ?? uid, uid) };
  }
  if (mode === "older") {
    return { ...cursor, lowUid: Math.min(cursor.lowUid ?? uid, uid) };
  }
  return {
    ...cursor,
    highUid: Math.max(cursor.highUid ?? uid, uid),
    lowUid: Math.min(cursor.lowUid ?? uid, uid),
  };
}

export function catchupIsComplete(
  allUids: number[],
  cursor: MailboxSyncCursor,
): boolean {
  if (cursor.highUid == null || cursor.lowUid == null) return false;
  return allUids.every((u) => u <= cursor.highUid! && u >= cursor.lowUid!);
}
...
async function imapAuthenticate(
  ...
  if (auth.type === "password") {
    const login = await session.command(
      `LOGIN ${quoteImapString(auth.username)} ${
        quoteImapString(auth.password)
      }`,
    );
    if (login.status !== "OK") {
      throw new ImapSyncError(
...
export async function fetchInboundFromImap(
  ...
  const result = await runImapInboxSync(options);
  return result.messages;
}

export async function runImapInboxSync(
  options: ImapFetchOptions,
  ...
  const deadline = Date.now() + Math.max(1, overallTimeoutMs);

  return await fetchInboundFromImapInner(
    options,
    connectTimeoutMs,
    commandTimeoutMs,
    deadline,
...
function parseFetchBlockToMessage(
  ...
  const bodyText = decoded.text.slice(0, maxBodyBytes);
  const bodyHtml = decoded.html
    ? decoded.html.slice(0, Math.max(maxBodyBytes, decoded.html.length))
    : null;
  return {
...
async function fetchInboundFromImapInner(
  ...
  let cursor = options.cursor ?? emptyMailboxSyncCursor();
  let timedOut = false;
  const messages: InboundImapMessage[] = [];
  let uidvalidity = cursor.uidvalidity ?? 0;
  try {
    await imapAuthenticate(session, auth);
    ...
    const parsedValidity = parseUidValidity(selected.untagged, selected.text);
    if (parsedValidity != null) {
      if (cursor.uidvalidity != null && cursor.uidvalidity !== parsedValidity) {
        cursor = emptyMailboxSyncCursor(parsedValidity);
      } else {
        cursor = { ...cursor, uidvalidity: parsedValidity };
      }
      uidvalidity = parsedValidity;
    } else if (!uidvalidity) {
      uidvalidity = cursor.uidvalidity ?? 1;
      cursor = { ...cursor, uidvalidity };
    }

    const search = await session.command("UID SEARCH ALL", "IMAP SEARCH");
    if (search.status !== "OK") {
      throw new ImapSyncError(
        "imap_search_failed",
    ...
    const allUids = extractUidList(search.untagged);
    const selectedUids = selectUidsForCycle(
      allUids,
      cursor,
      options.maxMessages,
    );
    if (selectedUids.mode === "idle") {
      cursor = {
        ...cursor,
        catchupComplete: catchupIsComplete(allUids, cursor) ||
          cursor.catchupComplete ||
          allUids.length === 0,
      };
      await session.command("LOGOUT").catch(() => undefined);
      return { messages, cursor, timedOut: false, uidvalidity };
    }

    const headerSection =
      "HEADER.FIELDS (FROM TO CC SUBJECT DATE MESSAGE-ID IN-REPLY-TO REFERENCES CONTENT-TYPE CONTENT-TRANSFER-ENCODING)";
    const fetchSpec =
      `UID INTERNALDATE BODY.PEEK[${headerSection}] BODY.PEEK[TEXT]<0.${
        Math.max(1, options.maxBodyBytes)
      }>`;

    const fetchOrder = [...selectedUids.uids].sort((a, b) => b - a);
    for (const batch of chunkUids(fetchOrder, IMAP_FETCH_BATCH_SIZE)) {
      if (Date.now() >= deadline) {
        timedOut = true;
        break;
      }
      try {
        const fetch = await withImapTimeout(
          session.command(
        ...
        for (const block of splitFetchBlocks(fetch.untagged)) {
          ...
          if (options.onProgress) {
            await options.onProgress({
              ...
              cursor,
              uidvalidity,
              mode: selectedUids.mode,
            });
          }
        }
      } catch (error) {
        if (error instanceof ImapSyncError && error.code === "timeout") {
          timedOut = true;
          break;
        }
        throw error;
    ...
    await session.command("LOGOUT").catch(() => undefined);
    return { messages, cursor, timedOut, uidvalidity };
  } finally {
    session.close();
  }
}
